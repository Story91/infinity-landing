...
const ProfileCardComponent = ({
  ...
  const finalAvatar = avatarUrl || image || '';
  ...
  const getOffsets = (evt: PointerEvent, el: HTMLElement) => { const r = el.getBoundingClientRect(); return { x: evt.clientX - r.left, y: evt.clientY - r.top }; };

  const handlePointerMove = useCallback((e: PointerEvent) => { const s = shellRef.current; if (!s || !tiltEngine) return; const { x, y } = getOffsets(e, s); tiltEngine.setTarget(x, y); }, [tiltEngine]);
                ...
                <div className="pc-user-info">
                  <div className="pc-user-details">
                    <div className="pc-mini-avatar">
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img src={miniAvatarUrl || finalAvatar} alt={`${name} mini`} loading="lazy" />
                    </div>
                  ...
                  <button className="pc-contact-btn" onClick={onContactClick} style={{ pointerEvents: 'auto' }} type="button">{contactText}</button>
                </div>
              )}
            </div>
